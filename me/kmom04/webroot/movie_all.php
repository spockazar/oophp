...
foreach($res as $val) {
  if($val->name == $genre) {
    ...
    $genres .= "<a href='" . $db->getQueryString(array('genre' => $val->name)) . "'>{$val->name}</a> ";
  }
}
...
$tr = "<tr><th>Rad</th><th>Id " . $db->orderby('id') . "</th><th>Bild</th><th>Titel " . orderby('title') . "</th><th>År " . $db->orderby('year') . "</th><th>Genre</th></tr>";
...
$glados['title'] = "Visa filmer med sökalternativ kombinerade";

$hitsPerPage = $db->getHitsPerPage(array(2, 4, 8), $hits);
